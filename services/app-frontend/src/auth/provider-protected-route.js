...
const ProviderProtectedRoute = ({ component, ...args }) => {
                    ...
                    if (result.status === 200) {
                        getAccessTokenSilently().then((JWTToken) => {
                            ...
                            if (actorTypeFromJwt !== null) {
                                if (actorTypeFromJwt === 'storageminer') {
                                    SetTenantValuesInSessionStorage(address, 'storageminer')
...
export default withRouter(ProviderProtectedRoute)
